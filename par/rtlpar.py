...
"""
# remove input|output [...]. leave only pin name
# output: name list
def PinLines2pinNames( pinsList ):
	pinList = []
	ptn_pinName = r'\s+([a-zA-Z0-9_]+)$'
	pinMatch = re.compile(ptn_pinName)
        for pinLine in pinsList:
		pinList.append(pinMatch.search(pinLine).group() )
